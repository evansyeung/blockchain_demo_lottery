...
def test_can_end_lottery():
  ...
  #Assert
  assert lottery.lottery_state() == 2 # Remember our lottery states inherently represent 0, 1, 2
...
def test_can_pick_winner_correctly():
  ...
  STATIC_RNG = 777
  # Now that we got the request_id, we can pretend to tbe the chainlink node and use this to call the callBackWithRandomness function
  get_contract("vrf_coordinator").callBackWithRandomness(request_id, STATIC_RNG, lottery.address, {"from": account})
  starting_balance_of_account = account.balance()
  balance_of_lottery = lottery.balance()
  # Assert
  # Randomness returns 777
  # 777 % 3 (players) = 0 so our account will be the winner
  assert lottery.recentWinner() == account
  # We transfer money to winner
